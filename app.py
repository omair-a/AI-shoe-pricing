...
with tab4:
    ...
    st.caption("How the AI analyses shoe data to calculate optimal pricing recommendations")
    ...
    with st.expander("🧠 How the AI Calculates the Optimal Price"):
        st.markdown("""
The system uses a machine learning model — **XGBoost** — to analyse hundreds of product records
and learn which combinations of product features support higher or lower prices in the market.

Rather than applying a fixed formula to every product, the model studies patterns across the entire
dataset and builds an understanding of how each feature influences pricing. When it encounters a
product, it weighs all of its characteristics together to arrive at a recommendation that reflects
the full context of that specific item.

For example, two products with identical costs and competitor prices can receive very different
optimal price recommendations if one is a **Nike Men's Running shoe on the West Coast in Spring**
and the other is a **Converse Youth Casual shoe in the South in Winter** — because the model has
learned that those feature combinations occupy very different positions in the market.

**The model never outputs a flat formula.** Every recommendation is the result of the model
processing all 12 features simultaneously and producing a price that reflects their combined influence.
        """)

        st.markdown("#### What Each Feature Contributes")

        feature_info = {
            "🏷️ Brand": "Captures brand pricing power. Premium brands like Nike and Brooks consistently support higher optimal prices than value brands like Skechers or Converse for comparable products.",
            "👟 Category": "One of the strongest signals. Boots and Hiking shoes occupy a fundamentally different price tier than Sandals or Casual shoes, regardless of cost.",
            "⚧ Gender": "Women's products typically support a slight price premium over Men's equivalents. Youth products are priced significantly lower, reflecting a different market segment.",
            "🍂 Season": "Seasonal demand drives pricing. Winter Boots command higher prices in Fall and Winter. Sandals peak in Summer. The model adjusts recommendations based on when a product is being sold.",
            "📍 Region": "Geographic pricing variation is real. West Coast and Northeast markets support higher prices due to cost of living. Southern and Midwestern markets are more price-sensitive.",
            "🗓️ Month": "A continuous signal that captures within-season timing, allowing the model to distinguish early-season vs. peak-season pricing dynamics.",
            "🧵 Material": "Premium materials like Leather and Gore-Tex support higher prices than Synthetic or Canvas alternatives.",
            "🎨 Color": "Certain colorways (e.g. Multicolor, limited-edition palettes) can command a premium over standard Black or White options.",
            "🏪 Store": "Retail channel matters. Products sold through Nike Store or Zappos have different pricing dynamics than those on Amazon or Kohl's.",
            "💰 Competitor Price": "The most direct market signal. The model anchors recommendations to what competitors are charging, adjusting up or down based on all other features.",
            "🏭 Our Cost": "Sets the absolute floor. No recommendation will ever result in a price that loses money after accounting for the minimum required margin.",
            "📊 Min Margin": "The profitability constraint. The model respects this boundary — it will never recommend a price that falls below the required gross margin for that product.",
        }

        for feature, explanation in feature_info.items():
            st.markdown(f"""
<div style="background:#13131f;border:1px solid #1e1e2e;border-left:3px solid #6c63ff;
            border-radius:8px;padding:12px 16px;margin-bottom:8px;">
  <strong style="color:#e8e4dc;font-size:0.92rem;">{feature}</strong><br>
  <span style="color:#c4c0d4;font-size:0.88rem;">{explanation}</span>
</div>""", unsafe_allow_html=True)
