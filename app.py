    ...
    st.dataframe(
        ...
        column_config={
            "SKU": st.column_config.TextColumn(
                "SKU",
                help="Stock Keeping Unit — a unique code that identifies each individual product. Format: BRAND-CATEGORY-GENDER-NUMBER",
            ),
            "Product": st.column_config.TextColumn(
                "Product",
                help="The full product name including brand, model, and gender segment.",
            ),
            "Brand": st.column_config.TextColumn(
                "Brand",
                help="The shoe brand (e.g. Nike, Adidas, Brooks). Brand affects pricing power and market positioning.",
            ),
            "Category": st.column_config.TextColumn(
                "Category",
                help="The type of shoe — e.g. Running, Basketball, Hiking, Casual, Boots, Sandals.",
            ),
            "Gender": st.column_config.TextColumn(
                "Gender",
                help="The target gender: Men's, Women's, Youth, or Unisex.",
            ),
            "Season": st.column_config.TextColumn(
                "Season",
                help="The season this product is associated with. Seasonal demand affects optimal pricing",
            ),
            "Region": st.column_config.TextColumn(
                "Region",
                help="The US region where this product is sold.",
            ),
            "Store": st.column_config.TextColumn(
                "Store",
                help="The retail store where the product is sold (e.g. Foot Locker, Amazon, DSW).",
            ),
            "Current Price": st.column_config.NumberColumn(
                "Current Price",
                help="The price we are currently charging for this product.",
                format="$%.2f",
            ),
            "Competitor Price": st.column_config.NumberColumn(
                "Competitor Price",
                help="The price a competitor is currently charging for the same product.",
                format="$%.2f",
            ),
            "Optimal Price": st.column_config.NumberColumn(
                "Optimal Price",
                help="The AI-recommended price for this product, calculated by XGBoost based on all product features.",
                format="$%.2f",
            ),
            "Adjustment": st.column_config.TextColumn(
                "Adjustment",
                help="The recommended price change — the difference between the Optimal Price and our Current Price.",
            ),
            "Change %": st.column_config.TextColumn(
                "Change %",
                help="The recommended price adjustment expressed as a percentage of the current price.",
            ),
            "Cur. Margin %": st.column_config.NumberColumn(
                "Cur. Margin %",
                help="Our current gross profit margin on this product, calculated as (Current Price - Cost) / Current Price.",
                format="%.1f%%",
            ),
            "Action": st.column_config.TextColumn(
                "Action",
                help="The recommended pricing action: Increase, Reduce, or Maintain.",
            ),
        },
    )
...
with tab4:
    ...
    with st.expander("🧠 How the AI Calculates the Optimal Price"):
        st.markdown("""
The AI calculates the optimal price using a supervised machine learning model built with XGBoost, a gradient boosting algorithm designed for structured data. Instead of relying on fixed pricing rules, the model learns patterns from historical product data to predict what price works best under different conditions.

The target variable, optimal price, is calculated using business constraints. Specifically, the target price is set to stay competitive with the market (typically at or slightly below the competitor price) while also respecting a minimum margin requirement to ensure profitability. This creates a realistic benchmark for the model to learn from.
